...
const ObiectivCuZile3 = () => {
  ...
  useEffect(() => {
    ...
    if (oras && oraStart !== '') {
      fetchRestaurant2Data();
    }
    if (oras && oraEnd !== '') {
      fetchRestaurant3Data();
    }
  }, [oras, pauza, oraStart, oraEnd]);

  useEffect(() => {
    const storedHistory = JSON.parse(localStorage.getItem('obiectivCuZile3ItineraryHistory')) || [];
  ...
  const handleGenerateItinerary = async () => {
    if (oras && oraStart !== '' && oraEnd !== '' && pretMaxim !== '') {
      try {
        if (!restaurant || !restaurant2 || !restaurant3) {
          throw new Error('Nu s-au găsit toate restaurantele.');
        }
        const oraStart2 = parseInt(oraStart) + 1;
        const oraEnd2 = parseInt(oraEnd) - 1;

        const response = await fetch(`http://localhost:8080/obiectiv/getItinerariuCuZile?oras=${oras}&oraStart=${oraStart2}&pauza=${pauza}&oraEnd=${oraEnd2}&pretMaxim=${pretMaxim}&zile=${zile}`);
        const data = await response.json();
        const updatedObiective = data.map((zi) => {
          const ziCuRestaurante = [];
          zi.forEach((obiectiv) => {
            if (obiectiv.ora_propusa < restaurant2.ora_propusa) {
              ziCuRestaurante.push(obiectiv);
            }
          });
          ziCuRestaurante.push(restaurant2);
          zi.forEach((obiectiv) => {
            if (obiectiv.ora_propusa >= restaurant2.ora_propusa && obiectiv.ora_propusa < restaurant.ora_propusa) {
              ziCuRestaurante.push(obiectiv);
            }
          });
          ziCuRestaurante.push(restaurant);
          zi.forEach((obiectiv) => {
            if (obiectiv.ora_propusa >= restaurant.ora_propusa && obiectiv.ora_propusa < restaurant3.ora_propusa) {
              ziCuRestaurante.push(obiectiv);
            }
          });
          ziCuRestaurante.push(restaurant3);
          zi.forEach((obiectiv) => {
            if (obiectiv.ora_propusa >= restaurant3.ora_propusa) {
              ziCuRestaurante.push(obiectiv);
            }
          });
          return ziCuRestaurante;
        });
        ...
        localStorage.setItem('obiectivCuZile3ItineraryHistory', JSON.stringify(storedHistory));
        setHistory(storedHistory);
  ...
  const fetchRestaurant2Data = async () => {
    try {
      const response = await fetch(`http://localhost:8080/restaurant/getRamdonByCategorieAndOra?oras=${oras}&categorie=Breakfast&ora=${oraStart}`);
      const data = await response.json();
      setRestaurant2(data);
    } catch (error) {
      console.error('A apărut o eroare în obținerea datelor restaurantului 2:', error.message);
    }
  };

  const fetchRestaurant3Data = async () => {
    try {
      const response = await fetch(`http://localhost:8080/restaurant/getRamdonByCategorieAndOra?oras=${oras}&categorie=Dinner&ora=${oraEnd}`);
      const data = await response.json();
      setRestaurant3(data);
    } catch (error) {
      console.error('A apărut o eroare în obținerea datelor restaurantului 3:', error.message);
    }
  };
                    ...
                    {zi.map((obiectiv, idx) => (
                      <Paper
                        ...
                        style={{
                          ...
                          backgroundColor: obiectiv === restaurant || obiectiv === restaurant2 || obiectiv === restaurant3 ? '#ffffcc' : 'transparent',
                        }}
                        key={idx}
                      >
...
export default ObiectivCuZile3;
